...
const envSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.NODE_ENV === "production") {
      cloudRes.error?.issues.forEach((i) =>
        ctx.addIssue({ ...i, path: ["COLLAB_DB_CLOUD_URI"] }),
      );
    } else if (data.NODE_ENV === "development") {
      localRes.error?.issues.forEach((i) =>
        ctx.addIssue({ ...i, path: ["COLLAB_DB_LOCAL_URI"] }),
      );
    }
  })
  .superRefine((data, ctx) => {
    ...
    if (data.NODE_ENV === "production") {
      cloudRes.error?.issues.forEach((i) =>
        ctx.addIssue({ ...i, path: ["YJS_DB_CLOUD_URI"] }),
      );
    } else if (data.NODE_ENV === "development") {
      localRes.error?.issues.forEach((i) =>
        ctx.addIssue({ ...i, path: ["YJS_DB_LOCAL_URI"] }),
      );
    }
  });
...
if (!result.success) {
  console.error(
    "There is an error with the environment variables:",
    result.error.issues,
  );
  process.exit(1);
}
...
const COLLAB_DB_URI = (
  NODE_ENV === "production" ? COLLAB_DB_CLOUD_URI : COLLAB_DB_LOCAL_URI
) as string;
const YJS_DB_URI = (
  NODE_ENV === "production" ? YJS_DB_CLOUD_URI : YJS_DB_LOCAL_URI
) as string;

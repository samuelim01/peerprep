...
import { createRoomInDB, createYjsDocument, findRoomByRoomId, findRoomsByUserId } from '../services/mongodbService';
...
export const getRoomIdsByUserIdController = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!rooms || rooms.length === 0) {
            return res.status(404).json({ error: 'No rooms found for the given user' });
        }
        ...
        return res.status(500).json({ error: 'Failed to retrieve room IDs by user ID' });
    }
};
...
export const getRoomByRoomIdController = async (req: Request, res: Response) => {
    try {
        ...
        return res.status(500).json({ error: 'Failed to retrieve room by room ID' });
    }
};